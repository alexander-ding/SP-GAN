...
import hier_execute as hier_ex
import utils
...
if __name__ == '__main__':
    hier = jp.parseJsonToHier(sys.argv[1], sys.argv[2])

    generate_program(hier)
    verts, faces = hier_ex.hier_execute(hier)
    utils.writeObj(verts, faces, "hier_out.obj")
    a = 1/0
    #q = [hier]
    ...
    while(len(q) > 0):
        ...
        GT = propsToProgram(node['cubes'][1:])
        GT.render('gt.obj')
